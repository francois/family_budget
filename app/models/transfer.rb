class Transfer < ActiveRecord::Base
  ...
  validate :presence_of_transaction_or_credit_account

  attr_accessible :debit_account, :credit_account, :transaction, :posted_on, :description, :amount
  ...
  def presence_of_transaction_or_credit_account
    if self.transaction.blank? then
      errors.add_on_blank("credit_account_id")
    else
      errors.add_on_blank("transaction_id")
    end
  end
end
