class Account < ActiveRecord::Base
  ...
  named_scope :by_most_debited, :select => "#{table_name}.*, SUM(#{Transfer.table_name}.amount) amount", :joins => "INNER JOIN #{Transfer.table_name} ON #{Transfer.table_name}.debit_account_id = #{table_name}.id", :order => "amount DESC", :group => "#{table_name}.id"
  named_scope :by_most_credited, :select => "#{table_name}.*, SUM(#{Transfer.table_name}.amount) amount", :joins => "INNER JOIN #{Transfer.table_name} ON #{Transfer.table_name}.credit_account_id = #{table_name}.id", :order => "amount DESC", :group => "#{table_name}.id"
